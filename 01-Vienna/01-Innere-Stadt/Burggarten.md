---
type: place
name: Burggarten
name_he: בורגגארטן
name_local: Burggarten
city: Vienna
country: Austria
district: Innere Stadt
visit_status: planned
content_status: first-complete-build
priority: medium
personal_rating: 4
tags: [place, vienna, garden, local-life]
hero_image:
  provider: wikimedia_commons
  storage: remote
  file: "Burggarten, Vienna.jpg"
  remote_url: "https://commons.wikimedia.org/wiki/Special:FilePath/Burggarten%2C%20Vienna.jpg?width=1600"
  source_page: "https://commons.wikimedia.org/wiki/File:Burggarten%2C%20Vienna.jpg"
  author: "Yair Haklai"
  license: "CC BY-SA 4.0"
  license_url: "https://creativecommons.org/licenses/by-sa/4.0/"
  verified: 2026-08-27
visit_duration:
  minimum_minutes: 20
  recommended_minutes: 40
nearby_places:
  - page: Neue-Burg
    name_he: נוייה בורג
    distance_m: 100
    walk_minutes: 2
  - page: Albertina
    name_he: אלברטינה
    distance_m: 200
    walk_minutes: 3
  - page: Heldenplatz
    name_he: כיכר הגיבורים
    distance_m: 250
    walk_minutes: 3
  - page: Vienna-State-Opera
    name_he: האופרה הממלכתית של וינה
    distance_m: 350
    walk_minutes: 5
  - page: Hofburg
    name_he: הופבורג
    distance_m: 450
    walk_minutes: 6
---
# בורגגארטן (Burggarten) - גן קיסרי לשעבר בלב העיר

![בורגגארטן (Burggarten)](https://commons.wikimedia.org/wiki/Special:FilePath/Burggarten%2C%20Vienna.jpg?width=1600)

*צילום: Yair Haklai. מקור: [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Burggarten%2C%20Vienna.jpg). רישיון: [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/).*

## למה לעצור

גן נעים בצמוד ל-Hofburg, טוב במיוחד אחרי ביקור פנימי ארוך. הוא היה בעבר גן פרטי קיסרי ונפתח לציבור לאחר נפילת המלוכה.

## הסיפור

גן בורגגארטן (Burggarten) נראה היום כמו הפסקה ירוקה ונעימה ליד הופבורג, אבל הוא נולד מתוך שינוי פוליטי וצבאי גדול. לאחר שהביצורים באזור נפגעו בתקופת המלחמות הנפוליאוניות, עוצב כאן גן חדש ששימש בתחילה את המשפחה הקיסרית ולא את הציבור הרחב. הגן היה חלק מהמרחב הפרטי של החצר, ולכן עצם הפיכתו לפארק ציבורי לאחר נפילת המלוכה מספרת משהו על המעבר מאימפריה לרפובליקה.

הגן קיבל במשך השנים אופי עירוני יותר, אך עדיין נשאר קשור מאוד למוסדות הקיסריים שסביבו. אנדרטת מוצרט, בית הדקלים והמבטים אל נוייה בורג יוצרים חיבור בין מוזיקה, אדריכלות וגינון בתוך שטח קטן יחסית. דווקא משום שאין כאן מוזיאון שדורש ריכוז או מסלול קשיח, קל להבחין בפרטים האלה בקצב רגוע.

הביקור בבורגגארטן מתאים במיוחד אחרי חללים סגורים של הופבורג או אלברטינה. המקום מזכיר שהעיר הקיסרית לא נבנתה רק מאולמות טקסיים, אלא גם מגנים פרטיים, שבילים ונקודות מנוחה שנועדו במקור לקבוצה מצומצמת מאוד של אנשים.

## מה לחפש

- אנדרטת [[Wolfgang-Amadeus-Mozart|וולפגנג אמדאוס מוצרט (Wolfgang Amadeus Mozart)]].
- Palmenhaus.
- מבטים אל Neue Burg ו-Albertina.

זה מקום טוב לזכור שגם קיסרים היו צריכים גינה, רק שהם קיבלו אחת במרכז העיר עם צוות תחזוקה שלא ביקש ועד בית.

## זמן שכדאי להקדיש

- **מינימום:** כ־20 דקות
- **מומלץ:** כ־40 דקות

## מה קרוב לכאן

- [[Neue-Burg|נוייה בורג (Neue Burg)]] — כ־100 מטר, כ־2 דקות הליכה. להבנת החלק המאוחר והמונומנטלי של Hofburg מול Heldenplatz. [קח אותי לשם](https://www.google.com/maps/dir/?api=1&destination=Neue+Burg%2C+Vienna%2C+Austria&travelmode=walking)
- [[Albertina|אלברטינה (Albertina)]] — כ־200 מטר, כ־3 דקות הליכה. לתצפית טובה על האופרה ולחיבור בין ארמון למוזיאון. [קח אותי לשם](https://www.google.com/maps/dir/?api=1&destination=Albertina%2C+Vienna%2C+Austria&travelmode=walking)
- [[Heldenplatz|כיכר הגיבורים (Heldenplatz)]] — כ־250 מטר, כ־3 דקות הליכה. כדי לראות את Hofburg בקנה מידה מונומנטלי ואת פרויקט ה-Kaiserforum שלא הושלם. [קח אותי לשם](https://www.google.com/maps/dir/?api=1&destination=Heldenplatz%2C+Vienna%2C+Austria&travelmode=walking)
- [[Vienna-State-Opera|האופרה הממלכתית של וינה (Wiener Staatsoper)]] — כ־350 מטר, כ־5 דקות הליכה. כדי לראות את מוסד האופרה כחלק מתכנון ה-Ringstrasse ולא רק כאולם מופעים. [קח אותי לשם](https://www.google.com/maps/dir/?api=1&destination=Wiener+Staatsoper%2C+Vienna%2C+Austria&travelmode=walking)
- [[Hofburg|הופבורג (Hofburg)]] — כ־450 מטר, כ־6 דקות הליכה. להמשך ישיר אל מרכז הכוח ההבסבורגי והחצרות ההיסטוריות. [קח אותי לשם](https://www.google.com/maps/dir/?api=1&destination=Hofburg%2C+Vienna%2C+Austria&travelmode=walking)

## מקורות
- [Wien.info](https://www.wien.info/en/see-do/parks-green-spaces/burggarten-344778)
